...
import { g as getCollection } from "./_slug__346ce338.mjs";
...
const myImage = {
  src: "/_astro/i-love-astro-weekly.bf4faa06.png",
  width: 658,
  height: 270,
  format: "png",
};
...
const $$Test = createComponent(
  async ($$result, $$props, $$slots) => {
    ...
    return renderTemplate`${renderComponent(
      ...
      {
        title: "Test",
        description: "a page for testing things",
        "data-astro-cid-6d7mwtum": true,
      },
      {
        default: ($$result2) =>
          renderTemplate`${renderComponent(
            ...
            { "data-astro-cid-6d7mwtum": true },
            {
              default: ($$result3) => renderTemplate`${renderComponent(
                $$result3,
                "Picture",
                $$Picture,
                {
              ...
              )}${maybeRenderHead()}<nav class="group rounded-xl mb-4 p-2 font-medium bg-slate-200 hover:bg-slate-100 text-slate-700 hover:text-slate-900" data-astro-cid-6d7mwtum>${[
                ["Home", "/"],
                ["Test", "/test"],
                ["Posts", "/posts"],
                ["About", "/about"],
                ...
                ([title, url]) =>
                  renderTemplate`<a${addAttribute(
                    url,
                    "href",
                  )} class="rounded-xl mx-2 px-3 py-2 font-medium bg-slate-200 hover:bg-zinc-300 text-slate-700 group-hover:text-zinc-800" data-astro-cid-6d7mwtum>${title}</a>`,
              )}</nav><h2 class="font-bold text-lg capitalize underline font-serif" data-astro-cid-6d7mwtum>
local frontmatter data
</h2><ul class="mb-12" data-astro-cid-6d7mwtum>${data1.map(
                (i) => renderTemplate`<li data-astro-cid-6d7mwtum>${i}</li>`,
              )}</ul><h2 class="font-bold text-lg capitalize underline font-serif" data-astro-cid-6d7mwtum>
fetch data from external API
</h2><ul class="mb-12" data-astro-cid-6d7mwtum>${data2
                .slice(0, 5)
                .map(
                  (i) =>
                    renderTemplate`<li data-astro-cid-6d7mwtum>${i.id} - ${i.title}<br data-astro-cid-6d7mwtum>${i.body}</li>`,
                )}</ul><h2 class="font-bold text-lg capitalize underline" data-astro-cid-6d7mwtum>
import local files: list of movies
</h2><ul data-astro-cid-6d7mwtum>${sortedData.map(
                (
                  movies,
                ) => renderTemplate`<li class="mb-4" data-astro-cid-6d7mwtum>${
                  movies.year
                } - <span class="underline" data-astro-cid-6d7mwtum>${
                  movies.title
                }</span>${" "}
starring ${joinWithAnd(
                  movies.starring,
                )}<br data-astro-cid-6d7mwtum>${" "}<span class="italic pl-8" data-astro-cid-6d7mwtum>${
                  movies.plot
                }</span></li>`,
              )}</ul><h2 class="font-bold text-lg capitalize underline" data-astro-cid-6d7mwtum>
import local files: list of popular cars
</h2><ul data-astro-cid-6d7mwtum>${carsJson.map(
                (cars) =>
                  renderTemplate`<li class="mb-4" data-astro-cid-6d7mwtum>${cars.year}&nbsp;${cars.make}&nbsp;${cars.model}</li>`,
              )}</ul><h2 class="font-bold text-lg capitalize underline" data-astro-cid-6d7mwtum>content collections</h2><ul data-astro-cid-6d7mwtum>${testColl.map(
                (item) =>
                  renderTemplate`<li class="mb-4" data-astro-cid-6d7mwtum>${
                    item.id
                  }. - ${item.data.name} - $${item.data.price} -${" "}${
                    item.data.color
                  } - ${item.data.brand} - ${item.data.category}</li>`,
              )}</ul><div class="shadow-mine1 flex flex-col mx-auto max-w-2xl mt-24 p-8 bg-slate-200" data-astro-cid-6d7mwtum><h2 class="font-mono font-bold text-gray-700 bg-gray-400 rounded-md block mb-4 pl-4" data-astro-cid-6d7mwtum>
.mine1
